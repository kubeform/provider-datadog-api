...
type SettingsLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.Settings, err error)
...
// List lists all Settingses in the indexer.
func (s *settingsLister) List(selector labels.Selector) (ret []*v1alpha1.Settings, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.Settings))
	})
	return ret, err
}
...
type SettingsNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.Settings, error)
...
// List lists all Settingses in the indexer for a given namespace.
func (s settingsNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.Settings, err error) {
...
// Get retrieves the Settings from the indexer for a given namespace and name.
func (s settingsNamespaceLister) Get(name string) (*v1alpha1.Settings, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("settings"), name)
	}
	return obj.(*v1alpha1.Settings), nil
}
